...
def test_go(dir: str, cli_path: str, rest: List[str], filter: Optional[str] = None, suite: Optional[str] = None) -> None:
    go = find_command('go', msg='go is required for testing')
    ...
    # Determine test package path
    if suite:
        # Check if suite exists as direct subdir (e.g., "integration", "tls")
        # or under unit/ (e.g., "namespace" -> "unit/namespace")
        if (basedir / suite).is_dir():
            test_path = f'./{suite}/...'
        elif (basedir / 'unit' / suite).is_dir():
            test_path = f'./unit/{suite}/...'
        else:
            raise RuntimeError(f"Suite not found: {suite}\n"
                               f"Available suites: integration, tls, or any directory under unit/")
    else:
        test_path = './...'

    args = [
        'test', '-timeout=1800s', '-bench=.', test_path,
        f'-binPath={binpath}',
        f'-cliPath={cli_path}',
...
if __name__ == '__main__':
    ...
    parser_test_go.add_argument('-s', '--suite', default=None,
                                help="run only tests in this suite (e.g., 'namespace', 'scripting', 'integration')")
